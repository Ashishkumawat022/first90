...
const Account = () => {
  ...
  const Profile = (responseData: any) => {
    ...
    notificationHandler(
      `Your teammate ${responseData.data?.firstName} has just updated their headshot and/or bio. Check out your team page!`,
      localData?._id
    );
    setTimeout(() => {
      localStorage.setItem("data", JSON.stringify(responseData.data));
  ...
  const notificationHandler = (msg: string, id: any) => {
    let data = {
      message: msg,
      userId: id,
    };
    ...
    notificationRequest(
      {
        url: `${process.env.REACT_APP_STUDENT_BASEURL}/notifications`,
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
